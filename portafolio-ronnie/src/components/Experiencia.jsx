  ...
  return (
    <div className="animate-fade-in max-w-5xl mx-auto py-8 px-4 sm:px-6">
      ...
      <h2 className="text-2xl sm:text-3xl font-bold mb-8 md:mb-10 text-white flex items-center gap-2 sm:gap-3 drop-shadow-md">
        <Briefcase className="text-blue-400 w-6 h-6 sm:w-8 sm:h-8" /> Trayectoria Profesional
      </h2>
      ...
      <div className="relative border-l-2 border-slate-700/50 ml-3 sm:ml-4 md:ml-6 space-y-8 md:space-y-12">
        {trabajos.map((trabajo, idx) => (
          <div key={idx} className="relative pl-6 sm:pl-8 md:pl-12 group">
            ...
            {/* Tarjeta de Experiencia con padding dinámico */}
            <div className={`bg-slate-900/60 backdrop-blur-md p-5 sm:p-6 md:p-8 rounded-2xl border border-slate-700/50 ${trabajo.sombra} hover:-translate-y-1 transition-all duration-300 shadow-xl`}>
              
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-3 sm:gap-4 mb-4">
                <div>
                  <h3 className="text-xl sm:text-2xl font-bold text-white mb-1 leading-tight">{trabajo.puesto}</h3>
                  <h4 className="text-base sm:text-lg font-medium text-slate-300 flex items-center gap-2">
                    {trabajo.empresa}
                  </h4>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2 text-slate-400 bg-slate-800/50 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-slate-700 w-fit">
                  <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  <span className="text-xs sm:text-sm font-medium whitespace-nowrap">{trabajo.periodo}</span>
                </div>
              </div>

              {/* Tecnologías con tamaño dinámico */}
              <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-5 sm:mb-6">
                {trabajo.tecnologias.map((tech, i) => (
                  <span key={i} className={`px-2.5 sm:px-3 py-1 text-[10px] sm:text-xs font-semibold rounded-full border ${trabajo.bgBadge}`}>
                    {tech}
                  </span>
                ))}
              </div>
