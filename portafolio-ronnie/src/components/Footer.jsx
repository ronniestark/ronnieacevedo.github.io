...
const Footer = ({ setActiveTab }) => {
  ...
  return (
    <footer className="w-full bg-[#0d1117] border-t border-slate-800/60 pt-16 pb-8 text-slate-300 mt-auto">
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
        ...
        {/* Columna 4: Contacto */}
        <div>
          <h3 className="text-white font-bold mb-4 uppercase tracking-wider text-sm invisible md:visible">Contacto</h3>
          <ul className="space-y-4">
            ...
            <li className="flex items-start gap-3">
              <Mail size={20} className="text-orange-500 flex-shrink-0 mt-0.5" />
              <span className="text-sm text-slate-400">ronniestark999@gmail.com</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      {/* Barra Inferior */}
      <div className="max-w-6xl mx-auto px-6 border-t border-slate-800/60 pt-6 flex flex-col md:flex-row justify-between items-center text-xs text-slate-500">
        <p>© {currentYear} Ing. Ronnie Acevedo. Todos los derechos reservados.</p>
        <p className="mt-2 md:mt-0">Construido con React & Tailwind CSS</p>
      </div>
    </footer>
  );
};
